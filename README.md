...
- `src/tiangong_ai_workspace/`: workspace Python package and CLI entrypoint.
  ...
  - `agents/`: LangGraph document workflows (`workflows.py`), dual-engine autonomous agents for LangGraph/DeepAgents (`deep_agent.py`), citation helpers (`citation_agent.py`), journal-band analysis (`journal_bands_agent.py`), and LangChain Tools with Pydantic input/output validation (`tools.py`).
