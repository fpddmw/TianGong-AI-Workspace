...
## Repository Layout
- `src/tiangong_ai_workspace/cli.py`: Typer CLI with `docs`, `agents`, `research`, `knowledge`, `embeddings`, `citation-study`, `mineru-with-images`, and `mcp` subcommands plus structured JSON output support.
...
- `uv run tiangong-workspace citation-study "large language model" --since-year 2020 --limit 30 --pdf ./paper.pdf` — 调用 OpenAlex + OpenAI 区分综述/研究并在图表摘要（自动抽取“Image Description”段落）加持下按高/中/低引用潜力分类。
